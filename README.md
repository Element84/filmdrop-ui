...
| SHOW_ITEM_AUTO_ZOOM      | If set to `true`, toggle will render in `Filters` list to let the user toggle if the map automatically centers on item footprint when selected item is changed. Default when initialized is auto-zoom not enabled, user must opt-in by turning on (choice will persist for app session).                                                                                                                                                                                                                                      | Optional |
