...
| UI_SOURCE_ID             | ID that can be used by backend services for debugging requests sent from specific frontend application depolyments to STAC API endpoints. If set, custom header will be passed as `X-FILMDROP-UI-SOURCE_ID` for all requests sent to backend STAC API. **IMPORTANT NOTE:** STAC API must be set to allow `X-FILMDROP-UI-SOURCE_ID` as a header value via CORS settings, else UI will not be able to load collections or perform searches.                                                                                     | Optional |
